...
function DrugProfile() {
    ...
    async function load() {
      ...
      if (filtered.length > 0) {
        ...
        const { data: reviewData } = await supabase
          ...
          .in('business_id', filtered.map((p) => p.business_id))
  ...
  const highestPrice = products.length ? Math.max(...products.map((p) => p.price)) : null
          ...
          <>
            ...
            <div style={{ border: `1px solid ${theme.border}`, borderRadius: 16, padding: 14, background: theme.cardBg, boxShadow: '0 1px 4px rgba(0,0,0,0.05)', marginBottom: 20 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
                ...
                <div>
                  <p style={{ margin: 0, color: theme.warning, fontSize: 14 }}>{'★'.repeat(Math.round(avgRating))}{'☆'.repeat(5 - Math.round(avgRating))}</p>
                  <p style={{ margin: 0, fontSize: 11.5, color: theme.textLight }}>{reviews.length} review{reviews.length !== 1 ? 's' : ''}</p>
                </div>
              </div>
